-- Migration 017: Enforce payment collection before repair delivery
-- Description: Updates private.update_repair_status RPC to reject transitions to DELIVERED when outstanding due > 0.01
...
BEGIN
    ...
    -- 6. PAYMENT-BEFORE-DELIVERY RULE ENFORCEMENT
    IF p_new_status = 'DELIVERED'::public.repair_status THEN
        SELECT COALESCE(SUM(amount), 0) INTO v_total_collected
        FROM public.repair_payments
        WHERE repair_id = p_repair_id;

        v_due := COALESCE(v_repair.service_revenue, 0) - v_total_collected;

        IF v_due > 0.01 THEN
            RAISE EXCEPTION 'Cannot mark repair as DELIVERED until full payment is collected. Total Amount: %, Collected: %, Due: %',
                v_repair.service_revenue, v_total_collected, v_due;
        END IF;
    END IF;

    -- 7. State machine workflow graph validation
    IF p_new_status = 'CANCELLED'::public.repair_status THEN
    ...
    -- 10. Attempt automated financial finalization if conditions met
    v_auto_finalized := private.try_auto_finalize_repair_financials(p_repair_id);

    -- 11. Build & return result JSON
